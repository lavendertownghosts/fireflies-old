<template>
  <div class="">
    <div
      class="relative text-black flex flex-col pb-10 border border-grey-light rounded"
    >
      <div
      ...
      <div v-show="activeView === 'days'" class="calendar-wrapper-inner-days">
        ...
        <div class="flex flex-wrap px-8">
          <div
            ...
            :key="index"
            class="text-base leading-16 w-1/7 text-center h-40 cursor-pointer flex items-center justify-center"
...
<script>
import { formatDateApi, calcNights, createDate } from '@/includes/utils'
...
export default {
  ...
  methods: {
    ...
    selectDate(date) {
      if (this.initValues[date.year] !== undefined) {
        if (this.initValues[date.year][date.month + 1] !== undefined) {
          let focus = this.initValues[date.year][date.month + 1]

          if (this.initValues[date.year][date.month] !== undefined) {
            focus = [...focus, ...this.initValues[date.year][date.month]]
          }
          if (this.initValues[date.year][date.month + 2] !== undefined) {
            focus = [...focus, ...this.initValues[date.year][date.month + 2]]
          }

          if (focus !== undefined) {
            for (let i = 0; i < focus.length; i++) {
              const dateFrom = createDate(focus[i].date_from)
              const dateTo = createDate(focus[i].date_until)
              const selected = new Date(date.year, date.month, date.day)

              if (
                dateFrom.getTime() <= selected.getTime() &&
                dateTo.getTime() >= selected.getTime() &&
                !this.isInPast({
                ...
                if (focus[i].reserved !== '1') {
                  this.selectedWeek = focus[i].weekofyear
                  this.$emit('select', {
                    id: focus[i].id,
                    year: dateFrom.getFullYear(),
        ...
        if (this.initValues[date.year][date.month + 1] !== undefined) {
          ...
          for (let i = 0; i < focus.length; i++) {
            ...
            if (
              ...
              if (this.selectedWeek === focus[i].weekofyear) {
                return ['bg-primary', 'text-white']
              } else if (
                focus[i].reserved === '1' &&
                dateTo.getTime() !== selected.getTime()
              ) {
                return ['bg-danger-opaque']
              } else if (dateTo.getTime() !== selected.getTime()) {
                return ['bg-primary-opaque']
              }
            }
          }
        }
