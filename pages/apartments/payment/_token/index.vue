...
export default {
  ...
  methods: {
    ...mapActions('apartments', ['paymentStatusV2']),
    paymentCompleted() {
      this.$router.replace(this.localePath('/apartments/reservation-success'))
    },
    paymentPending() {
      this.$router.replace(this.localePath('/apartments/reservation-pending'))
    },
    paymentError() {
      this.$router.replace(this.localePath('/apartments/reservation-failed'))
    }
  }
}
</script>
