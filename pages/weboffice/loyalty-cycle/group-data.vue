    ...
    <div v-else class="text-black">
      <WebofficeBreadcrumb
        :links="[
          ...
          {
            name: $t('weboffice.LoyaltyCycleGroupData')
          }
        ]"
      />
      <div class="flex items-center text-lg leading-24 font-bold mb-24 mt-16">
        ...
        ></i>
        {{ $t('weboffice.LoyaltyCycleGroupData') }}
      </div>
    </div>
          ...
          <thead class="font-bold">
            <tr>
              <template v-for="(header, index) in cycleGroup.tableHeader">
                <th :key="index" class="py-14 px-8 text-left">
...
export default {
  ...
  methods: {
    ...
    async getDetails(params) {
      ...
      this.selectedname = params.user !== undefined ? params.user : ''
      this.cycleGroup = await this.$store.dispatch(
        'user/getLoyaltyCycleGroup',
        request
      )
