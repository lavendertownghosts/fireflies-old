...
export default {
  ...
  methods: {
    ...mapActions('hotel', ['paymentStatus']),
    paymentCompleted() {
      this.$router.replace(this.localePath('/hotel/reservation-success'))
    },
    paymentPending() {
      this.$router.replace(this.localePath('/hotel/reservation-pending'))
    },
    paymentError() {
      this.$router.replace(this.localePath('/hotel/reservation-failed'))
    }
  }
}
</script>
