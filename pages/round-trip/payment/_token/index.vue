...
export default {
  ...
  methods: {
    ...mapActions('round-trip', ['paymentStatus']),
    paymentCompleted() {
      this.$router.replace(this.localePath('/round-trip/reservation-success'))
    },
    paymentPending() {
      this.$router.replace(this.localePath('/round-trip/reservation-pending'))
    },
    paymentError() {
      this.$router.replace(this.localePath('/round-trip/reservation-failed'))
    }
  }
}
</script>
