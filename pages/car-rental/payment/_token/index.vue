...
export default {
  ...
  methods: {
    ...mapActions('car-rental', ['paymentStatus']),
    paymentCompleted() {
      this.$router.replace(this.localePath('/car-rental/reservation-success'))
    },
    paymentPending() {
      this.$router.replace(this.localePath('/car-rental/reservation-pending'))
    },
    paymentError() {
      this.$router.replace(this.localePath('/car-rental/reservation-failed'))
    }
  }
}
</script>
