  ...
  <main class="flex-grow">
    ...
    <div v-show="!isLoading">
      ...
      >
        ...
        <div class="w-full flex flex-row items-stretch mt-16 md:mt-24 lg:mt-0">
          <div
            class="flex flex-col flex-shrink-0 hidden md:block w-grid-sm xl:w-grid-md"
          >
            <!-- NOTE: Thematics List -->
            <ThematicSidebarCategories
              v-for="(items, index) in thematics"
              :key="index"
              :thematic="items"
            />
            <div
              v-show="thematicsMore"
              class="cursor-pointer link text-sm mb-12"
              @click="getThematics()"
            >
              {{ $t('thematic.moreThematics') }}
            </div>
            <!-- NOTE: Filter forms -->
            <ThematicFilterForm
...
<script>
import placeholderBackgroundUrl from '~/assets/images/large-thematic-offer-header-small.jpg'
import backgroundUrl from '~/assets/images/large-thematic-offer-header.jpg'
...
import ThematicSidebarCategories from '@/components/thematic/ThematicSidebarCategories'
...
export default {
  ...
  methods: {
    ...
    async getThematics() {
      let res
      if (this.thematicsMore) {
        try {
          res = await this.$store.dispatch('thematic-offers/getThematicList', {
            page: 1,
            perpage: this.thematicsPerPage,
            lang: this.$i18n.locale,
            type: ['flashsale', 'superflashsale']
          })
        } catch (e) {
        ...
        if (res !== undefined) {
          if (res.data !== undefined) {
            if (res.data.result !== undefined) {
              if (this.thematicsPerPage === 99) {
                this.thematicsMore = false
              } else {
                this.thematicsMore = true
              }
              if (res.data.result.thematics.length <= 5) {
                this.thematicsMore = false
              }
              this.thematics = res.data.result.thematics
              this.thematicsAll = res.data.result.total
              this.thematicsPerPage = 99
            }
          }
        }
      }
  ...
  head() {
    return {
      title: this.$t('meta.FlashSaleTitle') + this.$t('meta.TitleSuffix'),
      meta: [
        {
          property: 'og:title',
          content: this.$t('meta.FlashSaleTitle') + this.$t('meta.TitleSuffix')
        },
        {
        ...
        },
        {
          ...
          content: this.$t('meta.FlashSaleDescription')
        },
        {
          hid: 'description',
          name: 'description',
